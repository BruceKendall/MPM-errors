...
In contrast, the birth rate (which we denote $b_x$ for age-structured demography and $b_i$ for stage-structured demography) is instantaneous: it is the average number of offspring produced at time $t$ by an individual that is alive at time $t$. 
...
Embracing this fertility coefficient requires overcoming cognitive dissonance---"juveniles" are reproducing!---and we have found that novices actively resist this.
...
However, in the "inutuitive" postbreeding census model (Fig. \@ref(fig:SScycles)c), the individuals that just matured won't reproduce until the end of the next timestep, a full timestep after their transition to adulthood. 
...
However, none of these are as straightforward as the re-indexing gambit for age structured models: they introduce changes such as giving the maturing juveniles adult survival rather than juvenile survival (which might or might not be desireable), or shortening the "juvenile" stage duration to account for the extra stage.
...
With repeated observations of marked individuals, one can simply derive this from the observations, for example, using multistate capture-mark-recapture models (CITE, although this is not guaranteed to be robust; see Discussion).
...
An even simpler approach is to simply say that, if the desired duration of stage $i$ is $T_i$ timesteps, then the fraction maturing is $1/T_i$. 
This effectively assumes that the age distribution is perfectly flat, which will only be true under asymptotic conditions if $\sigma_i / \lambda_1 = 1$.
