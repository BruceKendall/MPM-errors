...
We first focuce on the effects on $\lambda_1$ and then evaluate the effects on other endpoints.
...
In order to investigate the effects of the errors on $\lambda_1$ with the lionfish and American alligator matrices, we constructed stage structured population matrices that corrected the three types of errors (L5, AN3, and AS3 in Appendix A). 
...
The effects of these errors can be observed by comparing open (with survival) and filled (without survival) circles on Figure X. 
...
However, for both lionfish and alligator populations, the survival rate over the time step of a model was high (0.95 per month for lionfish and 0.83 per year for alligator). 
...
The effect was slightly higher for the Southern poulation of American alligator than the Northern popualtion. 
This resulted from a larger fecundity associated the Southern population (5.98 individuals per year) compared with the Northern popualtion (2.37 individuals per year), suggesting the error may be magnified for organisms with high fecundity.
...
The use of different maturation models (Table 2) has substantial effects on $\lambda_1$ (Fig. X). 
Inp articular, the asymptotic population growth rate was substantially overestimated under the FAS models compared with the AAS models. 
...
However, calculating other endpoints such as stable stage distribution, reproductive value, sensitivity, elasticity, gneration time, and damping ration are often the primary goal of constructing an MPM, especially for management applications or studies of life history theory. 
Here, we investigate the effects of the errors on these endpoints using matrices for the lionfish and two popualtions of Americcan alligators. 
...
For American alligator popualtions, we developed two additional stage-structured models that correct some or all of the problems and three age-structured matrices, of which two include the original errors associated with the fertility coefficients, for each population. 
All of these models were used to calculate $\lambda_1$, stable stage distribution, reproductive value, sensitivity and elasticity of $\lambda_1$ to stage-specific survival rate and fecundity, damping ratio, and generation time, and the results are shown in Appendix A. 
Here, we highlight some of the results. 

The effects of failure to include survival terms in the fertility coefficients can be observed by comparing the age-structured matriceswith and without the correction. 
The first set of age-structured matrices (L6, AN4, AS4) included corrected fertility coefficients with corresponding suurvival rates multiplied to the original fertility. 
When the survival rate was missing in teh ferility coefficient, asymptotic popualtion growth rate was always overestimated, adult stable stage distrubiton was underestimated (Figs. A.2. and A.9), and adult reproductive value was overesitmated (Figs. A.8 and A.16).
The reduced adult stable stage distrubituion and increased adult reproductive value resulted from the fact the models mistakenly assumed more offsprimg were born, inflating the number of younger individuals. 
...
In all cases, the missing ferility coefficient caused the overestimation of adult stage distribution (Figs. A.2 and A.9), and underestimation of adult reproductive value (Figs. A.3 and A.10). 
...
In essence, this advice boils down to "think like a modeller."
In our experience, non-modellers require a lot of interactive tutorial examples to really learn this process.
...
One pedagogical approach that may be helpful is to build "seasonal" submodels, and use matrix multiplication to generate the integration over the full timestep (rather than requiring the integration to be done in the brain of the model builder).
