...
Then the reproduction by newly maturing individuals is $F_{x_m} = P_{x_m} b_{x_m}$ (we use $P$ for survival to clarify that it is a differently indexed parameter from $\sigma$ as used above); the cognitive dissonance is finessed by "hiding" the fact that these individuals started the timestep as juveniles.
...
The simplest stage-structured model has newborns, a nonreproductive juvenile class that spans multiple timesteps, and reproductive adults (Fig. 2).
**Make SS lifecycle graphs**
...
We will refer to these as the "asymptotic age structure" (AAS) model, the "stationary age structure" (SAS) model (where "stationary" means that the population is neither growing nor declining), and the "flat age structure" (FAS) model.
...
FecCorrect.glm <- glm(!FecCorrect ~ YearPublication, data = mpm_data, 
                      family = "binomial")
FecCorrect.perc <- round(mean(mpm_data$FecCorrect, na.rm = TRUE) * 100)
FecCorrect.n <- sum(!is.na(mpm_data$FecCorrect))
FecCorrect.P <- round(summary(FecCorrect.glm)$coef[2,4], 3)
FecCorrect.trend <- if (FecCorrect.P > 0.05) {
  "did not change"
} else if (coef(FecCorrect.glm)[2] > 0) {
  "increased"
} else {
  "decreased"
}
```
We were able to unambiguously identify the components of the fecundity term in `r FecCorrect.n` studies.
`r FecCorrect.perc`% of these studies failed to include an appropriate survival component in the reproduction terms.
The frequency of these errors `r FecCorrect.trend` over time ($P =$ `r FecCorrect.P`; Fig. \@ref(fig:regplots)a).

First reproduction

Maturation

```{r regplots, fig.cap=cap}
p <- ggplot(mpm_data, aes(YearPublication, as.numeric(!FecCorrect))) + 
  geom_jitter(width = 0.05, height = 0, shape = "|", size = 2) +
  geom_smooth(method = "gam",  method.args = list(family = "binomial", k=3)) +
  theme_classic() + xlim(2010.95, 2015.05) + xlab("Publication year") +
  ylab("Frequency of erroneous reproduction term")
print(p)
cap <- 'Trends in matrix population model construction errors through time.'
```
