...
Damping ratio is given by $\lambda_1/\left|\lambda_2\right|$, where $\lambda_1$  is the eigenvalue of the largest magnitude and $\lambda_2$  is that of the second largest, and measures how quickly transient dynamics dissipate over time. 
...
In this study, we used the mean age of mothers to represent the generation time [@Bienvenu2015]. 
For generation time calculations, the use of age-structured models over stage-structured models is generally recommended [@Fujiwara2017]. 
There are also substantial variations among the stage-structured models.
For example, generation time under L1-L3, which contain more errors, was substantially underestimated than that under L4-L5. 
Generation time also varied among stage-structured models for alligator populations A1-A3 and A5-A7. 
