...
abstract: |
  ...
  Of the studies where stages may last longer than one time step, 25% constructed the maturation rate using inappropriate formulas, and 37% used methods that were either undescribed or would generate the correct development time only if the population remained at the observed age-within-stage structure. 
...
Furthermore, the students, when sent to find a published model to analyze, have uncovered a surprising number of published MPMs that have failed to meet these challenges. 
...
Many animals and plants satisfy this assumption, and this allows us to avoid the extra complications associated with using discrete-time models to represent a continuously breeding species. 
...
But there is an important difference: survival (which we denote $\sigma_x$ for Leslie models and $\sigma_i$ for Lefkovitch models, where $x$ is the age and $i$ is stage) represents the fraction of individuals in a class ($x$ or $i$) that survive for a full timestep, from time $t$ to time $t+1$. 
...
A model that has the first fertility coefficient associated with age $x_m$ (Fig. \@ref(fig:lifecycles)c) results in the modeled age at first reproduction being $x_m + 1$, a year delay relative to the actual life history.
...
This is particularly well illustrated in Fig. 7.3 of @Mills2007.
...
If stage durations are fixed (e.g., all individuals mature after two years as juveniles), then the goal is to create a "stage-from-age" model [e.g., @Ebert1998, Ch. 8] where the mean stage duration in the model equals the fixed stage duration in the life history.
...
The second is the asymptotic growth rate, which determines the degree to which one year's entering cohort is larger or smaller than the previous one under the stable age or stage distribution.
This makes $\gamma$ non-trivial to calculate, as the resulting formula (Table \@ref(tab:matmodels)) involves $\lambda_1$, which can only be calculated once the MPM has been constructed and calibrated on data!
...
As long as the modeled age-within-stage distribution remains the same as it was during the observation period, then the model will get the mean stage duration correct.
...
The modeled life history consists of three stages (larvae, juvenile, and adult), and time step of the model was one month. The original model was a post-breeding census model, but it did not include the survival of adults in the fertility rate. 
...
We developed four stage-structured population models that gradually correct for some or all of these problems and three age-structured matrices, of which two include the original errors associated with the fertility coefficients (Supplementary Material). 
We used the eight models to calculate $\lambda_1$, stable stage distribution, reproductive value, sensitivity and elasticity of $\lambda_1$ to stage-specific survival rate and fecundity, damping ratio, and generation time (Supplementary Material). 

For American alligator populations, @Dunham2014 developed two stage-structured matrix population models to compare the status of northern and southern populations, which differed in stage durations and birth rates. 
The original models consist of five stages (eggs, larvae, juvenile, subadults, and adults), and the time step of the model was one year.
...
We developed two additional stage-structured models that corrects some or all of these problems and three age-structured matrices, of which two include the original errors associated with the fertility coefficients, for each population. 
The six population models for each population were used to calculate the same quantities that we calculated with the lionfish models (Supplementary Material). 
...
The effects of these errors can be observed by comparing the age-structured matrices with and without the correction (Supplementary Material). 
...
When the survival rate was missing in the fertility coefficient, asymptotic population growth rate was always overestimated (Figure S1 and S8), adult stable stage distribution was underestimated (Figure S2 and S9), and adult reproductive value was overestimated (Figures S8 and S16). 
The reduced adult stable stage distribution and increased adult stable stage distribution resulted from the fact the models mistakenly assumed more offspring were born, inflating the number of younger individuals. 
...
In all cases, the missing fertility coefficient caused the underestimation of asymptotic population growth rate (Figures S1 and S8), overestimation of adult stage distribution (Figures S2 and S9), and underestimation of adult reproductive value (Figures S3 and S10). 
...
Use of different maturation models (Table 2) has substantial effects on demographic statistics, and these effects can be observed by comparing the FAS (L3), SAS (L4), AAS (L5), and age-structured (L8) models for lionfish populations. 
The asymptotic population growth rate was substantially overestimated under the FAS and SAS models compared with the AAS and age-structured models (Figure S1), with latter two having the same growth rate estimate. 
Although the stable stage distributions of adults under the four models appear similar (Figure S2), the stable stage distributions of juveniles were underestimated with the FAS and SAS models compared with the age structured models (Figure S2), suggesting that the stable stage distribution of the larval stage was overestimated under the two stage-structured models. 
...
This results from the fact the estimated population growth rate was below 1 for the northern population and above 1 for the southern population. 
In other words, the SAS model would magnify the deviation in asymptotic population growth rate from 1. 
When the asymptotic growth rate is less than 1, the SAS model also underestimated the adult stable stage distribution (Figure S9) and overestimated the adult reproductive value (Figure S10). 
The opposite effects are observed when the asymptotic growth rate was greater than 1. 

## Effects on sensitivity, elasticity, damping ratio, and generation time
As described above, the three common errors in constructing matrix population models can affect population growth rate, stable stage distribution, and reproductive value, which are the three basic statistics associated with a population matrix. 
...
For lionfish population, the rank order of both sensitivity and elasticity among stages changed from L3 to L4 as we changed the way we calculate the transition rates (Figures S6).
...
Therefore, any bias associated with these two quantities affect the estimated generation time (Figures S7 and S16). 
We found substantial and variable impacts of MPM construction errors on the estimated generation time.
In addition, estimated generation time in general tends to be biased with stage-structured models [@Fujiwara2017].
These results suggest the calculation of generation time needs to be done carefully. 
...
The fact they deviate from each other (cf. Figures S1 and S5,  Figures S8 and S15) suggests the estimation of $\lambda_2$ is also affected by the errors in constructing population matrices. 
...
## Results
*Note: The data collection by the Compadrinos is ongoing. Here we report an analysis based on data collected for the initial feasibility study for the project.*
...
Based on our analysis of the COMADRE animal matrix model database [@Salguero-Gomez2016], we find that these errors are quite common: XX% of published models have at least one error, and YY% contain all three. [WRITE CODE TO GENERATE THESE]
Furthermore, there is no evidence that these errors are becoming less frequent through time.
...
Second, incorrect specification of the maturation rate seems to have the largest impact on these statistics.
Third, even a stage-structured model that correctly matches the mean time to maturity gives incorrect transient dynamics and generation time if the age at maturation is actually fixed.
...
While a more complete understanding of these biases might be derived by analyzining more models, spanning a range of life histories, ecological understanding and management decisions would be better served by constructing the models correctly in the first place.

Why do these errors occur? 
The specific errors probably have different sources, but our experience working with students and non-modelers suggests that some confusion arises from inconsistent terminology and notation, and a lack of clear guidance on maturation rates.
However, a relatively new development in educational theory, the idea of the "threshold concept" [@Meyer2006], provides some useful insights as to why the correct construction of MPMs seems to be so challenging.
In a succinct overview of the concept, @Cousin2006 points out that key features of a threshold concept are that it "is transformative because it involves an ontological as well as a conceptual shift [and] is integrative in that it exposes the hidden interrelatedness of phenomenon."
She goes on to point out that a threshold concept

> is likely to involve forms of ‘troublesome knowledge’; David Perkins defines this as ‘that which appears counter-intuitive, alien (emanating from another culture or discourse), or seemingly incoherent’ [in @Meyer2003a, p.7]. From this view, mastery of a threshold concept can be inhibited by the prevalence of a ‘common sense’ or intuitive understanding of it. Getting students to reverse their intuitive understandings is also troublesome because the reversal can involve an uncomfortable, emotional repositioning.    

\noindent Several issues related to translating biological understanding to models seem to fit this description.
...
First, different texts use different words to describe the birth rate ($b_x$) and the fertility coefficient ($F_x$)---"fertility," "fecundity," and "reproduction" are used to represent $F_x$ in different texts, and the terms have different scientific meanings in human demography and various fields of animal ecology. 
In particular, many animal ecologists use the term "fertility" to refer to the potential to have offspring, and so associate it with $b_x$ rather than $F_x$.
...
The failure to include reproduction when an individual first matures, which is solely found in postbreeding census models, is related to a true threshold concept: the counter-intuitive need to assign postive fertility coefficients to some juveniles.
This is probably exacerbated by the fact that most textbooks devote the most thorough explanation of MPM construction to age-structured models (where the confusing nature of postbreeding census models can be hidden by labeling an individuals age class as its age at its *next* birthday).
...
Most textbooks do not treat this issue, and those that do [e.g., @Caswell2001; @Ebert1998] tend to be more mathematical than many nonmodelers may be comfortable with. 
...
This creates a model that makes correct projections under both transient and asymptotic states, and can be used both for projecting the population and for calculating life history statistics.
...
The clarity certainly requires clear and consistent notation and terminology, as well as attention to the knowledge level of the audience, but that will probably not be sufficient.
...
The biologically-based intuitions that underly incorrect formulations of MPMs may have the strength of scientific misconceptions, in which case the training material will need to actively uproot them before a better understanding of the model requirements can be learned.

Experts must also address the fact that many creators of animal MPMs are not comfortable with even the simple programmng required to calculate quantities like the AAS development coefficient.
...
Both of these issues are addressed by formulas for birth-flow models.
Another challenge arises when demography is heterogeneous within a stage class, as when individuals grow continuously through life and survial and birth rates are size dependent.
@Fujiwara2017 have studied this and provide new recommendations for how best to calculate "average" demographic rates within a heterogenous stage class.

Plant MPMs, while prone to qualitatively similar types of errors, differ in detail.
In almost all cases, "newborns" are seeds; in many species, newly produced seeds have the potential to transition to two qualitativly different stages (seedlings and seed bank) in the following timestep.
...
Most plant MPMs have at least some classes that are defined by size rather than stage.
...
In addition, transition rates between size classes are usually estimated from the observed number of transitions; but the latter depend on the distributions of size within each size class (analagous to the distribution of age within stage discussed in the current paper), which in turn depend on the recent history of population fluctuations.
The general solution for the latter problem is to avoid size classes altogether and instead use an integral projection model [@Ellner2016a]; but the creation of IPMs requires some programming skill.
...
- Paragraph about how AAS, SAS, and FAS models are related to each other [MF]. **Masami: do we still need this, or are your concerns covered by the new table?**
...
However, transitioning from an age-structured model (which is what all textbooks use for a detailed explanation of MPMs) to a stage-structured model creates major challenges, at least for post-breeding census models.
