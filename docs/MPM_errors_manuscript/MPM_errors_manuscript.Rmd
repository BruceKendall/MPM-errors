...
For lionfish population, the rank order of both sensitivity and elasticity among stages changed as we changed the way we calculate the transition rates (Fig. \@ref(fig:lfelasfig)).
This implies that conservation strategy can be affected by the errors in developing a population matrix.   

```{r lfelasfig, fig.asp=1, fig.cap='Effects of MPM construction errors on the elasticity of $\\lambda_1$ to stage-specific survival in the lionfish model. (a) Contrasts among the three maturation models, using correct values for the fertility coefficients. (b) Contrasts between approaches to fertility coefficient constructon, using the (correct) AAS maturation model.'}
...
fert_names <- c("correct",
                "delayed maturation",
                "delayed maturation and missing survival")
lf2 <- mutate(lf2,
