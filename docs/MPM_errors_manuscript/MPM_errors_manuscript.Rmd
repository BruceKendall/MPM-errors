...
In general, we expect that stage-structured modles will have higher damping ratios than the equivalent age-structured models, as the effects of maturation time delays are reduced.
...
The specific errors probably have different sources, but our experience working with students and non-modellers suggests that prime culprits are inconsistent terminology and notation, a lack of clear guidance on maturation rates, and a "threshold concept" (CITE) associated with postbreeding census models.

The failure to include a survival component in the fertility coefficient is most surprising, as all textbooks make clear statements about the need for this. We suspect that the causes are threefold. First, different texts use different words to describe the birth rate ($b_x$) and the fertility coefficient ($F_x$)---"fertility," "fecundity," and "reproduction" are used to represent $F_x$ in different texts, and the terms have different scientific meanings in human demography and various fields of animal ecology. In particular, many animal ecologists use the term "fertility" to refer to the potential to have offspring, and so associate it more with $b_x$.
