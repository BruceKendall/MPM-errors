...
This relationship between the mean residence time (stage duration) and the fraction departing (maturing) is true for a simple Markovian departure process (REFERENCE - any stochastic process book will work **[MASAMI: Can you pick one?, Gallager, R.G. 1995 Discrete Stochastic Processes, Kluwer Academic Publishers, Norwell, Massachusetts ]**). 
...
We developed four stage-structured population models that gradually correct for some or all of these problems and three age-structured matrices, of which two of them include the original errors associated with fertility coefficient (Supplementary Material). 
Along with the original models, we used the eight models to calculate $\lambda_1$, stable stage distribution, reproductive value, sensitivity and elasticity of $\lambda_1$ to stage-specific survival rate and fecundity, damping ratio, and generation time (Supplementary Material). 
...
We developed two additional stage-structured models that corrects some or all of these problems and three age-structured matrices, of which two of them include the original errors associated with fertility coefficient, for each population. 
...
The fertility coefficients in both lionfish and alligator models in the original papers omitted the survival of adults. The effects of these errors can be observed by comparing the age-structured matrices with and without the correction (Supplementary Material). The first set of age-structured matrices (L6, A4, and A10) were converted from the original stage-structured matrices, which were missing the survival rate of adults. Another set of age-structured models (L8, A6, and A12) included corrected fertility coefficients with corresponding survival rates multiplied to the original fertility. When the survival rate was missing in the fertility coefficient, asymptotic population growth rate was always overestimated (Figure S1 and S8), adult stable stage distribution was underestimated (Figure S2 and S9), and adult reproductive value was overestimated (Figures S8 and S16). The reduced adult stable stage distribution and increased adult stable stage distribution resulted from the fact the models mistakenly assumed more offspring were born, inflating the number of younger individuals. 

For both lionfish and alligator populations, the survival rate over the time step of a model was high (0.95 per month for lionfish and 0.83 per year for alligator). Therefore, the effects were relatively minor. However, when we deal with organisms with low adult survival rate or when the time step is extended (e.g. time step is changed to 1 year in the lionfish model), a fertility coefficient could be substantially inflated without adult survival rate being included, causing biases in demographic statistics. 
...
For example, the effects were greater for the southern alligator population than northern alligator population because the duration from egg to adult stages was longer for northern population, causing less number of individuals to transition into adult stage each year, and the fecundity was lower for the northern population, causing less number of individuals to be born from each parent. 
Both of these would cause the reproduction in the first year to be smaller, reducing the effects of the missing fertility coefficient to be small. The effects of the missing fertility coefficient is expected to be large for species for which the contribution of offspring from individuals reproducing the first time is large. 
...
Use of different maturation models (Table 2) has substantial effects on demographic statistics, and these effects can be observed by comparing FAS model (L3), SAS model (L4), AAS model (L5), and age-structured model (L8) for lionfish populations. 
The asymptotic population growth rate was substantially overestimated under FAS and SAS models compared with the AAS and age-structured models (Figure S1), with latter two having the same growth rate estimate. Although stable stage distributions of adults under the four models appear similar (Figure S2), stable stage distributions of juveniles were underestimated with FAS and SAS models compared with the age structured models (Figure S2), suggesting the stable stage distribution of the larval stage was overestimated under the two stage-structured models. 
The reproductive value of adults were substantially underestimated with FAS and SAS models compared with AAS or the age-structured models.

The original models for alligator populations used SAS model (A2 and A8 for the northern and southern populations after correcting other problems). 
Therefore, we compared it with AAS model (A3 and A9 for the northern and southern populations, respectively) and age-structured models (A6 and A12 for the northern and southern populations, respectively). 
Similarly to the lionfish models, AAS and age-structured models gave the same asymptotic population growth rate. 
However, SAS models underestimated the population growth rate for the northern population, and overestimated it for the southern populations. 
...
When the asymptotic growth rate is less than 1, SAS model also underestimated adult stable stage distribution (Figure S9) and overestimated adult reproductive value (Figure S10). 
...
The three common errors in constructing matrix population models can affect population growth rate, stable stage distribution, and reproductive value, which are the three basic statistics associated with a population matrix. Calculating the sensitivity (or elasticity) of $\lambda_1$ to changes in matrix elements or underlying demographic parameters is often the primary goal of constructing and MPM, especially for management applications or studies of life history theory. 
...
This suggests that overestimates of the maturation rate should reduce the sensitivity of $\lambda_1$ to both stasis and growth terms, thus reducing the sensitivity of $\lambda_1$ to survival. 
...
We found substantial variations in the estimated generation time among the models.
In addition to the errors in constructing matrix population models, estimated generation time in general tends to be biased with stage-structured models [@Fujiwara2017].
These results suggest the calculation of generation time needs to be done carefully. 

Damping ratio is given by $\lambda_1/\left|\lambda_2\right|$, where $\lambda_1$  is the eigenvalue of the largest magnitude and $\lambda_2$  is that of the second largest, and it measures how quickly transient dynamics dissipate over time. 
...
C.  Why does this happen? Most texts are incomplete (especially for stage-structured models) and/or have inconsistent or confusing notation. Talk about @Caswell2001 as example. Also, these are "threshold concepts."
...
E.  Recommendations for experts:
    1. Develop handbooks and training materials that are both comprehensive and clear to novices (noting that issues related to threshold concepts cannot be solve simply by clearing up notation)
...
## C. Why are these errors so common?
...
In particular, many animal ecologists use the term "fertility" to refer to the potential to have offspring, and so associate it more with $b_x$.
