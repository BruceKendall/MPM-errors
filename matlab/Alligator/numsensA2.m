...
for PAR = 1:5
    for j=1:100
            ...
            fn=P(j,5,PAR);
            A=zeros(4,4);
            P2=sum((s2).^[0:d2n-2])/sum((s2).^[0:d2n-1])*s2;
            G2=(s2).^(d2n-1)/sum((s2).^[0:d2n-1])*s2;%
            P3=sum((s3).^[0:d3n-2])/sum((s3).^[0:d3n-1])*s3;
            G3=(s3).^(d3n-1)/sum((s3).^[0:d3n-1])*s3;%
            A(2,1)=s1;
            A(2,2)=P2;
            A(3,2)=G2;
            A(3,3)=P3;
            A(4,3)=G3;
            A(4,4)=s4;
            A(1,3)=s3^(9/12)*s0*fn*(s3).^(d3n-1)/sum((s3).^[0:d3n-1]);
            A(1,4)=s4^(9/12)*s0*fn;
